...
将 iMessage 与 AI 平台集成，支持**阿里云百炼（Bailian）**和 **Dify 工作流**，实现自动接收和智能回复 iMessage 消息。

## 功能特点

- **实时消息监控**：使用文件系统事件监控 iMessage 数据库变化，秒级响应新消息
- **AI 智能回复**：支持阿里云百炼应用 API 或 Dify 工作流处理消息，均支持多轮对话上下文
- **多用户会话隔离**：为每个联系人独立维护会话历史，互不干扰
- **触发机制灵活**：消息以 `@` 开头时触发 AI 回复，避免误触
- **多平台支持**：一键切换阿里云百炼或 Dify 工作流作为 AI 后端
- **联网搜索支持**（百炼）：可选启用 DuckDuckGo 联网搜索，获取实时信息
- **Markdown 自动清理**：AI 回复自动去除 Markdown 格式，适配 iMessage 纯文本
- **群聊支持**：支持识别群聊消息并正确回复
- **Web 管理界面**：基于 Flask 的简洁配置面板，支持平台切换、日志查看和会话管理
...
1. 前往 [Dify](https://cloud.dify.ai) 创建一个**工作流（Workflow）**类型的应用
2. 在工作流编辑器的**开始节点**中定义一个文本输入变量（如 `query`）
3. 在应用发布页面获取 **API 密钥**（Workflow API Key）
4. 将 **基础 URL**、**API 密钥**和**输入变量名**填入本应用的 Web 配置页面

> **注意**：Dify 工作流的输入变量名必须与你在 Dify 中配置的完全一致，默认为 `query`。

### 可选配置项

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| AI 平台 | 选择百炼或 Dify 工作流 | 百炼 |
| 检查间隔 | 后台强制检查的间隔（秒） | 10 |
| 强制检查间隔 | 即使使用文件监控也定期全面检查的间隔（秒） | 60 |
| 监控模式 | 文件系统事件监控 / 轮询 | 文件监控 |
| 联网搜索 | 启用 DuckDuckGo 搜索增强回复（仅百炼） | 开启 |
| 思考模式 | 启用深度思考模式（仅百炼） | 关闭 |

## 工作原理

1. `imessage_reader.py` 通过 `watchdog` 监控 `~/Library/Messages/chat.db` 的文件变化
2. 当检测到新消息时，筛选以 `@` 开头的消息内容
3. 去除 `@` 前缀后，根据配置的 AI 平台调用对应 API（百炼应用 API 或 Dify 工作流）获取回复
4. 通过 `osascript` 执行 AppleScript 自动发送 iMessage 回复
5. 每个联系人的对话历史和会话 ID 独立保存在 `user_sessions.json` 中
